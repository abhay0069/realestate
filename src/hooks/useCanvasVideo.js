...
export function useCanvasVideo(canvasRef, frameCount = 240) {
    ...
    // Calculate progress based on real loading, but gate completion on min time
    const realProgress = (loadedCount / frameCount) * 100;

    return {
        // If images load fast, we still want to show 100 only when min time elapsed
        progress: realProgress,
        isLoading: loadedCount < frameCount || !minTimeElapsed,
        drawFrame
    };
}
